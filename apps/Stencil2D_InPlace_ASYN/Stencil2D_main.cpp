...
int main(int argc, char *argv[])
{
	...
	uint64_t nCUsPerCluster = strtoul(argv[5],NULL,0);
	uint64_t nClusters= strtoul(argv[6],NULL,0);
	...
	if (affin.generateMask()){
		for(size_t i=0; i<N_REPES;++i){
			...
			rt.run(launch<Stencil2DRowDecomposition>(InitialMatrix,N_ROWS,N_COLS,N_TSTEPS, &Runtime::finalSignal)); 
...
//	//std::cout<<avgTimeSeq<<std::endl;
//	
//	std::cout << N_ROWS<<"*"<<N_COLS<<","<<g_nCU<<","<<g_nSU<<","<<g_nSU*(1+g_nCU)<<","<<N_TSTEPS<<","<<std::setprecision(18)<< AvgTime_k<<","<<AvgTime_w<<","<<avgTimeSeq<<std::endl;
	bool iscorrect;
