...
rm Stencil2DInPlaceAsynTpsTestResult.csv
nCores=48
nSUs_size=(1 2 4)
ts_size=(1 5 20)
nCUs_step=(48 4 2 1)
...
echo "MatrixDim,nCU,nSU,TotalCores:(nSU*(1+nCU)),num.TimeSteps,Kernel Times(ns),Whole Times(ns)" >> Stencil2DInPlaceAsynTpsTestResult.csv;
...
#	
#		echo "MatrixDim,nCU,nSU,TotalCores:(nSU*(1+nCU)),num.Repeats,Kernel Times(ns),Whole Times(ns)" >> Stencil2DInPlaceAsynTpsTestResult.csv;
#		for ((ts=1;ts<101;++ts))
#		do
#			./Stencil2D_InPlace_Asyn_Tps $n_rows $n_cols $ts $n_reps $nCUs $nSUs >> Stencil2DInPlaceAsynTpsTestResult.csv
#		done
