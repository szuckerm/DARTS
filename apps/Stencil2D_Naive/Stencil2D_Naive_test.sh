...
rm Stencil2DNaiveTestResult.csv
nCores=48
nSUs_size=(1 2 4)
ts_size=(1 5 20)
...
echo "MatrixDim,nCU,nSU,TotalCores:(nSU*(1+nCU)),num.TimeSteps,Kernel Times(ns),Whole Times(ns)" >> Stencil2DNaiveTestResult.csv;
...
#	for ((nCUs=1;nCUs<${nCUs_size};nCUs=$[${nCUs}*2]))
		...
		do
			...
			do
				...
				#	echo "MatrixDim,nCU,nSU,TotalCores:(nSU*(1+nCU)),num.Repeats,Kernel Times(ns),Whole Times(ns)" >> Stencil2DNaiveTestResult.csv;
					./Stencil2D_Naive $n_rows $n_cols $ts $n_reps $nCUs $nSUs >> Stencil2DNaiveTestResult.csv
...
#		for ((ts=1;ts<101;++ts))
#		do
#			./Stencil2D_InPlace_Asyn_Tps $n_rows $n_cols $ts $n_reps $nCUs $nSUs >> Stencil2DNaiveTestResult.csv
#		done
#	done
#
#
